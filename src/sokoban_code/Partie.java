...
/**
 * Classe permettant de gerer une partie
 */
public class Partie {
	// Attributs
	...
	public void lancerNiveau(int niv) {
		...
		int k = 0; // L'entier k permet de numeroter les caisses et de les manipuler dans le
					// tableau les contenant
		...
		try {
			...
			m_c = new Caisse[m_nbCaisses]; // En lancant un nouveau niveau, la liste m_c (contentant les caisses)
										// doit etre construite en fonction du nombre de caisse m_nbCaisses du
										// niveau
			for (int i = 0; i < m_nbCaisses; i++)
				m_c[i] = new Caisse();
	...
	public char getPlateauElt(int i, int j) {
		return m_plat.getElt(i, j);
	}
	 ...
	 * @return entier contenant le numero de la caisse si la case visee la contient
	 *         et -1 sinon
	 */
	 ...
	 * 
	 * @return True si le niveau est finit (chaque cible contient une caisse) et
	 *         False sinon
	 */
