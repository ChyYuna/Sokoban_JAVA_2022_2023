...
/**
 * Classe permettant de creer un plateau
 */
public class Plateau extends Objet {
	 ...
	 * @param x       : entier contenant la coordonnee x du plateau
	 * @param y       : entier contenant la coordonnee y du plateau
	 * @param fichier : ifstream du fichier contenant les elements du plateau du
	 *                niveau en cours
	 */
	public Plateau(int x, int y, Scanner scanner) {
		...
		for (int i = 0; i < m_longueur; i++) {
			...
			for (int j = 0; j < m_largeur; j++) {
				tab[j][i] = new ObjetImmobile(j, i, line_type.charAt(j)); // x , y, comme sur un plan (ne pas penser
																// matriciellement)
			}
		}
	}
	...
	// Methodes
	/**
	 * Donne acces aux elements du plateau (contenus dans un tableau
	 * d'ObjetImmobile)
	 * 
	 * @return Objet de type ObjetImmobile**
	 */
	public ObjetImmobile[][] getTab() {
		return tab;
	}

	public char getElt(int i, int j) {
		return tab[i][j].getType();
	}
	 ...
	 */
	public void setTab(ObjetImmobile[][] T) {
		tab = T;
	}
